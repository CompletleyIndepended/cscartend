...
//
    ...
    $user_names = get_fn_ln();
    $users_name_l_f = fn_get_username($users_names);
    // fn_print_die($users_name_l_f);
    ...
    fn_filters_handle_search_result($params, $users, $search);

    fn_gather_additional_products_data($users, [
        // 'get_icon'      => true,
        // 'get_detailed'  => true,
        // 'get_options'   => true,
        // 'get_discounts' => true,
...
function get_fn_ln(){
    // $id_users = db_get_array('SELECT b.firstname, b.lastname FROM ?:departments a LEFT JOIN ?:users b ON a.user_id = b.user_id');
    // $id_users = db_get_array('SELECT ?:departments.user_id FROM departments LEFT JOIN ?:users b ON a.user_id = b.user_id ON');
    $id_users = db_get_array('SELECT ?:departments.user_id FROM ?:departments LEFT JOIN ?:users ON ?:departments.user_id = ?:departments.department_id');
    return $id_users;
}

// function get_name_users($user_names){
//     $firstname = db_get_field('SELECT b.firstname b.lastname FROM ?:users b WHERE b.user_id = ?i', $user_names);
//     return $firstname;
// }

function fn_get_username($user_id) {
    $user_firstname = db_get_field('SELECT firstname FROM ?:users WHERE user_id = ?i', $user_id);
    $user_lastname = db_get_field('SELECT lastname FROM ?:users WHERE user_id = ?i', $user_id);
